...
function applyMutation(
  ...
  switch (mutation.type) {
    case 'create_node': {
      ...
      return {
        ...graph,
        nodes: { ...graph.nodes, [mutation.node.id]: mutation.node },
    ...
    case 'delete_relationship': {
      return {
        ...graph,
        relationships: graph.relationships.filter((rel) => rel.id !== mutation.relationshipId),
        updatedAt: Date.now(),
      };
    }
    case 'update_node_type_index': {
      const currentIndex = graph.nodeTypes[mutation.nodeType] || [];
      const newIndex =
        mutation.operation === 'add'
          ? [...new Set([...currentIndex, mutation.nodeId])]
          : currentIndex.filter((id) => id !== mutation.nodeId);
      return {
        ...graph,
        nodeTypes: { ...graph.nodeTypes, [mutation.nodeType]: newIndex },
        updatedAt: Date.now(),
      };
    }
    default:
...
export function applyMutationsToGraph(
  ...
  return mutations.reduce(applyMutation, graph);
}
