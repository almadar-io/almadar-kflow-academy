...
import { useLayerGoal } from '../hooks/useLayerGoal';
...
const LearningGoalDisplay: React.FC<LearningGoalDisplayProps> = ({
  goal,
  layerNumber,
  graphId,
  onGoalUpdated,
  ...
  const { saveGoal, isSaving } = useLayerGoal({
    graphId: graphId || '',
    layerNumber,
    initialGoal: goal,
    onGoalUpdated,
  ...
  const handleSave = async () => {
    if (!graphId) {
      console.warn('Cannot save goal: graphId is missing');
      return;
    }

    try {
      await saveGoal(editedGoal);
      setIsEditing(false);
    } catch (error) {
...
export default LearningGoalDisplay;
