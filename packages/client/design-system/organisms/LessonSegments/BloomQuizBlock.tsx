...
import { CodeBlock } from '@design-system/molecules/markdown/CodeBlock';
      ...
      <div className="font-semibold text-indigo-900 dark:text-indigo-200 mb-3 space-y-2">
        {questionSegments.map((segment: { type: 'markdown' | 'code'; content: string; language?: string }, idx: number) =>
          segment.type === 'markdown' ? (
            ...
            <CodeBlock key={`q-code-${idx}`} language={segment.language || 'text'} code={segment.content} />
          )
        )}
      </div>
        ...
        <div className="rounded-lg bg-white/80 dark:bg-gray-800/80 p-3 text-sm text-slate-800 dark:text-gray-200 shadow-sm border border-indigo-100 dark:border-indigo-800 mt-3 space-y-2">
          ...
          {answerSegments.map((segment: { type: 'markdown' | 'code'; content: string; language?: string }, idx: number) =>
            segment.type === 'markdown' ? (
              ...
              <CodeBlock key={`a-code-${idx}`} language={segment.language || 'text'} code={segment.content} />
            )
          )}
        </div>
      )}
