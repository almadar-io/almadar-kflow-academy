 ...
 *
 * Thin wrapper over the canonical `buildExpansionMutations` core (kept async + with the
 * same signature so the streaming controller is unchanged). All concept/parent/milestone
 * logic lives in the core so streaming and non-streaming paths can't drift.
 */
export async function parseProgressiveExpandContent(
  ...
  return buildExpansionMutations(content, graph, mutationContext, learningGoal, numConcepts);
}
